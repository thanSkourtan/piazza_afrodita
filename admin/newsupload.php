<?php
    ...
    if($_POST['action']=='delete'){
            ...
            $query1 = "SELECT image FROM news WHERE id = $id";
            ...
            $id= $_GET['id'];
            #update the word
            $q="UPDATE news SET title='$title',body = '$body' ,user_id=$user WHERE id = $id";
        ...
        if ($uploadOk == 0) {
            header("Location:index.php?page=news&error=0");
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["inputFile"]["tmp_name"], $target_file)) {
                ...
                $q="INSERT INTO news (title,body,image,user_id) VALUES ('$title','$body','$filename',$userId)";
                $r = mysqli_query($dbc,$q);

                echo mysqli_error($dbc);
                print_r($_POST);

                header("Location:index.php?page=news&error=1");
